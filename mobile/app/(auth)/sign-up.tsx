  ...
  const handleVerification = async () => {
    ...
    try {
      ...
      if (signUp.status !== "complete") {
        //console.log("Signup is not complete:", signUp.status);
        return;
      }
      // -------------------------
      ...
      console.error("Verification exception:", JSON.stringify(error, null, 2));
    } finally {
      setLoading(false);
    }
  };
    ...
    <View className="flex-1 justify-center px-6">
      {!verificationStarted ? (
        <>
          ...
          <Link href="/sign-in" className="text-center mt-5">
            Already have an account? Login
          </Link>
        </>
      ) : (
        <>
          <Text className="text-3xl font-bold mb-3">Verify Email</Text>
